...
plot_richness(cus_kraken2_physeq, x= "Tissue", color="Subject",measures = c("Shannon", "Simpson", "InvSimpson"))+
  geom_point(size=5) +
   theme_classic()+
  theme(legend.key.size = unit(1.0, "cm"), legend.text = element_text(size=13),legend.title = element_text(size=14,face="bold"), strip.text = element_text(face="bold",size=12), axis.text.y = element_text(face="bold", angle=-90,size=9),axis.text.x = element_blank(),axis.title = element_blank()) + guides(color = guide_legend(override.aes = list(size=5)))
...
## Plot Alpha Diversity for three software together
```{r}
library(gridExtra)
alpha_diversity_kraken2$sample <- rownames(alpha_diversity_kraken2) 
Kraken2_alpha <- alpha_diversity_kraken2 %>% pivot_longer(c(Shannon, Simpson, Inverse_Simpson), names_to = "alpha_index", values_to="Kraken2" )

alpha_diversity_clark$sample <- rownames(alpha_diversity_clark) 
clark_alpha <- alpha_diversity_clark %>% pivot_longer(c(Shannon, Simpson, Inverse_Simpson), names_to = "alpha_index", values_to="CLARK" )

alpha_diversity_clarks$sample <- rownames(alpha_diversity_clarks) 
clarks_alpha <- alpha_diversity_clarks %>% pivot_longer(c(Shannon, Simpson, Inverse_Simpson), names_to = "alpha_index", values_to="CLARK-s" )

joinalpha1 <- left_join(Kraken2_alpha,clark_alpha)
combined_alpha <- left_join(joinalpha1,clarks_alpha)

combined_alpha_annotated <- combined_alpha %>% pivot_longer(c(Kraken2,CLARK,`CLARK-s`), names_to="software", values_to="value") %>% mutate(Tissue = sapply(sample, function(x){
  if(substr(x,nchar(x),nchar(x)) == "K"){
    "Kidney"
  }else if(substr(x,nchar(x),nchar(x)) == "S"){
    "Spleen"
  }else if(substr(x,nchar(x),nchar(x)) == "L"){
    "Lung"
  }else{
    x
  }
})) %>% mutate(Subject = sapply(sample, function(x){
  substr(x, 1, 3)
}))
combined_alpha_annotated$software <- factor(combined_alpha_annotated$software, levels=c("Kraken2","CLARK","CLARK-s"))
combined_alpha_annotated$alpha_index <- factor(combined_alpha_annotated$alpha_index, levels=c("Shannon","Simpson","Inverse_Simpson"))

p1 <- ggplot(combined_alpha_annotated %>% subset(alpha_index=="Shannon") ,aes(x=Tissue, y = value)) +
         geom_point(aes(color=Subject),size=2.5)+
  facet_grid(software~alpha_index, scales = "free_y", switch = "y")+
  theme_bw() +
  labs(y= "Alpha Index", x = "Subject's Tissue")+
  theme(axis.title = element_text(size=10),axis.title.x = element_text(size=10,color="white"), axis.text = element_text(size=10), strip.text = element_text(size=10), legend.text = element_text(size=10), legend.title = element_text(size=10), strip.placement = "outside",legend.position = "none")


p2 <- ggplot(combined_alpha_annotated %>% subset(alpha_index=="Simpson") ,aes(x=Tissue, y = value)) +
         geom_point(aes(color=Subject),size=2.5)+
  facet_grid(software~alpha_index, scales = "free_y", switch = "y")+
  theme_bw() +
  labs(y= "Alpha Index", x = "Subject's Tissue")+
  theme(axis.title = element_text(size=10),axis.title.y = element_text(size=10,color = "white"), axis.text = element_text(size=10), strip.text.y = element_blank(), legend.text = element_text(size=10), legend.title = element_text(size=10), strip.placement = "outside",legend.position = "none", strip.text = element_text(size=10))

p3 <- ggplot(combined_alpha_annotated %>% subset(alpha_index=="Inverse_Simpson") ,aes(x=Tissue, y = value)) +
         geom_point(aes(color=Subject),size=2.5)+
  facet_grid(software~alpha_index, scales = "free_y", switch = "y")+
  theme_bw() +
  labs(y= "Alpha Index", x = "Subject's Tissue")+
  theme(axis.title = element_text(size=10,color="white"), axis.text = element_text(size=10), strip.text.y = element_blank(), legend.text = element_text(size=10), legend.title = element_text(size=10), strip.placement = "outside",legend.position = "none",strip.text = element_text(size=10))

combined_alpha_plots <- grid.arrange(p1,p2,p3,ncol=3)

ggsave("/Users/rachel/Dropbox/5.Rachel-projects/Metagenomic_Analysis/final_analysis/combined_alpha_index.pdf", combined_alpha_plots)
```

### Plot beta diversity together
```{r}

mds_kraken2_meta$software <- "Kraken2"
mds_clark_meta$software <- "CLARK"
mds_clarks_meta$software <- "CLARK-s"

combined_beta <- rbind(mds_kraken2_meta,mds_clark_meta,mds_clarks_meta)

pb1 <- ggplot(combined_beta%>%subset(software=="Kraken2"), aes(x= MDS1, y=MDS2))+
  geom_point(aes(color=Tissue),size=2.5) +
  geom_label_repel(aes(label=samples),
                  point.padding = 0.5,size=2)+
  facet_wrap(~software,ncol=1,strip.position = "right")+
  theme_classic()+
  theme(strip.placement = "outside", axis.text = element_text(size=10), strip.text = element_text(size = 10), axis.title = element_text(size=10),legend.title = element_text(size=10),legend.text = element_text(size=10),legend.position = "none")
  

pb2<- ggplot(combined_beta%>%subset(software=="CLARK"), aes(x= MDS1, y=MDS2))+
  geom_point(aes(color=Tissue),size=2.5) +
  geom_label_repel(aes(label=samples),
                  point.padding = 0.5,size=2)+
  facet_wrap(~software,ncol=1,strip.position = "right")+
...
pb3 <- ggplot(combined_beta%>%subset(software=="CLARK-s"), aes(x= MDS1, y=MDS2))+
  geom_point(aes(color=Tissue),size=2.5) +
  geom_label_repel(aes(label=samples),
                  point.padding = 0.5,size=2)+
  facet_wrap(~software,ncol=1,strip.position = "right")+
...
combined_beta_plot <- grid.arrange(pb1,pb2,pb3, ncol=2)
ggsave("/Users/rachel/Dropbox/5.Rachel-projects/Metagenomic_Analysis/final_analysis/combined_beta_index.pdf", combined_beta_plot)


```

### Plot Significant Abundant Taxa together
```{r}
sigtax_kraken2_lineage$software <- "Kraken2"
clark_deseq_lineage$software <- "CLARK"
clarks_deseq_lineage$software <- "CLARK-s"

ps1 <- ggplot(sigtax_kraken2_lineage, aes(x=Genus, y=log2FoldChange, color= Phylum))+ geom_point(size=3) + 
  facet_wrap(~software)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5,size = 9),legend.key.size = unit(0.6, "cm"), legend.text = element_text(size=9),legend.title = element_text(size=9), axis.text.y = element_text(size=9),axis.title.y  = element_blank(), axis.title = element_blank(),legend.position = "none",axis.title.x = element_text(size=12,color = "white"))+
  scale_color_manual(values=sig_phylum_color)

ps2 <-ggplot(clark_deseq_lineage, aes(x=Genus, y=log2FoldChange, color= Phylum))+ geom_point(size=3) +
  facet_wrap(~software)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5,size = 9),legend.key.size = unit(0.6, "cm"), legend.text = element_text(size=9),legend.title = element_text(size=9), axis.text.y = element_text(size=),axis.title.y =  element_blank(),  axis.title.x = element_text(size=12),legend.position = "none")+
  scale_color_manual(values=sig_phylum_color)

ps3 <-ggplot(clarks_deseq_lineage, aes(x=Genus, y=log2FoldChange, color= Phylum))+ geom_point(size=3) +
  facet_wrap(~software)+
  theme_classic()+
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5,size = 9),legend.key.size = unit(0.6, "cm"), legend.text = element_text(size=9),legend.title = element_text(size=9), axis.text.y = element_text(size=9),axis.title.x =  element_text(size=36,color="white"),axis.title.y=element_blank(), legend.position = "none")+
  scale_color_manual(values=sig_phylum_color)

combined_sigtax_plot <- grid.arrange(ps1,ps2,ps3,ncol=3)

ggsave("/Users/rachel/Dropbox/5.Rachel-projects/Metagenomic_Analysis/final_analysis/combined_sigtax.pdf", combined_sigtax_plot)
